         ...
         <v-card>
            ...
            <v-card-actions>
               ...
               <v-btn
                  ...
                  :disabled="disable_buttons"
                  :loading="disable_buttons"
                  @click.prevent = "delete_event_types(event_type_id)"
               >
                  DELETE
               </v-btn>
            </v-card-actions>
...
export default {
   ...
   data: () => ({
      ...
      //    });
      // },
      delete_event_types(id) {
         console.log("From Dialog Component" + id)
         this.disable_buttons = true

         axios.delete( "/api/events/types/destroy/" + id )
            .then((res) => {
               console.log(res.data)
               this.dialog = false
               this.disable_buttons = false
               this.$emit('event_type_deleted', { deleted_id: id, delete_message: res.data} )
            }).catch(error => {
               this.dialog = false
               this.disable_buttons = false
               this.$emit('event_type_delete_failed', id)
            })
