         ...
         <v-card>
            ...
            <v-card-actions>
               ...
               <v-btn
                  ...
                  :disabled="disable_buttons"
                  :loading="disable_buttons"
                  @click.prevent = "delete_people_section(people_section_id)"
               >
                  DELETE
               </v-btn>
            </v-card-actions>
...
export default {
   ...
   data: () => ({
      ...
      //    });
      // },
      delete_people_section(id) {
         console.log(id)
         this.disable_buttons = true

         axios.delete( "/api/people/destroy/" + id )
            .then((res) => {
               console.log(res.data)
               this.dialog = false
               this.disable_buttons = false
               this.$emit('people_section_deleted', { deleted_id: id, delete_message: res.data} )
            }).catch(error => {
               this.dialog = false
               this.disable_buttons = false
               this.$emit('people_section_delete_failed', id)
            })
